...
export const LastMinuteCard: React.FC<LastMinuteCardProps> = ({ entry, daysUntilDeparture }) => {
  ...
  const getBadgeText = (): string => {
    const priceDropText = getPriceDropBadgeText(entry.currentPrice, entry.previousPrice);
  ...
  return (
    ...
    >
      ...
      <Chip
        icon={entry.isDeal ? <LocalFireDepartmentIcon fontSize="small" /> : <AccessTimeIcon fontSize="small" />}
        label={getBadgeText()}
        color={entry.isDeal ? reasonColor : daysUntilDeparture <= 7 ? 'error' : 'default'}
        size="small"
        sx={{
          position: 'absolute',
          top: -10,
      ...
      <CardContent sx={{ pb: 1 }}>
        ...
        {entry.tripLengthDays > 0 && (
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
            ⏱️ {t('lastMinute.tripLengthDays', { count: entry.tripLengthDays })}
          </Typography>
        )}
        ...
        <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
          ...
          {entry.previousPrice && entry.previousPrice > entry.currentPrice && (
            <Typography
              variant="body2"
              sx={{
